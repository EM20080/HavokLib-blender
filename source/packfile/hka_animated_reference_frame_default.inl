...
struct Interface {
  ...
  hkaAnimatedReferenceFrame::Interface BasehkaAnimatedReferenceFrame() const {
    int16 off = m(Members::basehkaAnimatedReferenceFrame); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  Vector4A16 Up() const { return m(Members::up) == -1 ? Vector4A16{} : *reinterpret_cast<Vector4A16*>(data + m(Members::up)); }
  Vector4A16 Forward() const { return m(Members::forward) == -1 ? Vector4A16{} : *reinterpret_cast<Vector4A16*>(data + m(Members::forward)); }
  ...
  Pointer<Vector4A16> ReferenceFrameSamplesPtr() {
    int16 off = m(Members::referenceFrameSamples); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  Vector4A16 *ReferenceFrameSamples() {
    int16 off = m(Members::referenceFrameSamples); if (off == -1) return nullptr;
  ...
  uint32 NumReferenceFrameSamples() const { return m(Members::numReferenceFrameSamples) == -1 ? uint32{} : *reinterpret_cast<uint32*>(data + m(Members::numReferenceFrameSamples)); }
  void Up(Vector4A16 value) { if (m(Members::up) >= 0) *reinterpret_cast<Vector4A16*>(data + m(Members::up)) = value; }
  void Forward(Vector4A16 value) { if (m(Members::forward) >= 0) *reinterpret_cast<Vector4A16*>(data + m(Members::forward)) = value; }
  void Duration(float value) { if (m(Members::duration) >= 0) *reinterpret_cast<float*>(data + m(Members::duration)) = value; }
  void NumReferenceFrameSamples(uint32 value) { if (m(Members::numReferenceFrameSamples) >= 0) *reinterpret_cast<uint32*>(data + m(Members::numReferenceFrameSamples)) = value; }
