...
struct Interface {
  ...
  Pointer<hkaAnimation> AnimationPtr() {
    int16 off = m(Members::animation); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  hkaAnimation *Animation() {
    int16 off = m(Members::animation); if (off == -1) return nullptr;
  ...
  Pointer<int16> TransformTrackToBoneIndicesPtr() {
    int16 off = m(Members::transformTrackToBoneIndices); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  int16 *TransformTrackToBoneIndices() {
    int16 off = m(Members::transformTrackToBoneIndices); if (off == -1) return nullptr;
  ...
  uint32 NumTransformTrackToBoneIndices() const { return m(Members::numTransformTrackToBoneIndices) == -1 ? uint32{} : *reinterpret_cast<uint32*>(data + m(Members::numTransformTrackToBoneIndices)); }
  ::BlendHint BlendHint() const { return m(Members::blendHint) == -1 ? ::BlendHint{} : *reinterpret_cast<::BlendHint*>(data + m(Members::blendHint)); }
  Pointer<int16> FloatTrackToFloatSlotIndicesPtr() {
    int16 off = m(Members::floatTrackToFloatSlotIndices); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  int16 *FloatTrackToFloatSlotIndices() {
    int16 off = m(Members::floatTrackToFloatSlotIndices); if (off == -1) return nullptr;
    if (layout->ptrSize == 8) return *reinterpret_cast<int16**>(data + off);
    return *reinterpret_cast<es::PointerX86<int16>*>(data + off);
  }
  ...
  uint32 NumFloatTrackToFloatSlotIndices() const { return m(Members::numFloatTrackToFloatSlotIndices) == -1 ? uint32{} : *reinterpret_cast<uint32*>(data + m(Members::numFloatTrackToFloatSlotIndices)); }
  Pointer<char> SkeletonNamePtr() {
    int16 off = m(Members::skeletonName); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  char *SkeletonName() {
    int16 off = m(Members::skeletonName); if (off == -1) return nullptr;
  ...
  Pointer<int16> PartitionIndicesPtr() {
    int16 off = m(Members::partitionIndices); if (off == -1) return {nullptr, lookup};
    return {data + off, lookup};
  }
  int16 *PartitionIndices() {
    int16 off = m(Members::partitionIndices); if (off == -1) return nullptr;
    if (layout->ptrSize == 8) return *reinterpret_cast<int16**>(data + off);
    return *reinterpret_cast<es::PointerX86<int16>*>(data + off);
  }
  ...
  uint32 NumPartitionIndices() const { return m(Members::numPartitionIndices) == -1 ? uint32{} : *reinterpret_cast<uint32*>(data + m(Members::numPartitionIndices)); }
  void NumTransformTrackToBoneIndices(uint32 value) { if (m(Members::numTransformTrackToBoneIndices) >= 0) *reinterpret_cast<uint32*>(data + m(Members::numTransformTrackToBoneIndices)) = value; }
  void BlendHint(::BlendHint value) { if (m(Members::blendHint) >= 0) *reinterpret_cast<::BlendHint*>(data + m(Members::blendHint)) = value; }
  void NumFloatTrackToFloatSlotIndices(uint32 value) { if (m(Members::numFloatTrackToFloatSlotIndices) >= 0) *reinterpret_cast<uint32*>(data + m(Members::numFloatTrackToFloatSlotIndices)) = value; }
  void NumPartitionIndices(uint32 value) { if (m(Members::numPartitionIndices) >= 0) *reinterpret_cast<uint32*>(data + m(Members::numPartitionIndices)) = value; }
